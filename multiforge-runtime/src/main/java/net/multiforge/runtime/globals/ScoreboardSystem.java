...
public final class ScoreboardSystem extends AbstractGlobalSystem {
     ...
     * plus the broadcast) onto the global region's mailbox. Safe to call
     * from any thread, at any time — not just during {@link #tick}.
     *
     * @return {@code true} if the mutation was accepted for routing,
     *         {@code false} if there is no known live {@link RegionId}
     *         yet (see {@link #isReady()}) — callers must run {@code
     *         mutation} themselves in that case rather than lose it.
     */
